import Game.Levels.Subset.L07subtrans
...
Introduction
"
The symbol `¬` means \"it is not the case that\".  To enter it,
type `\\not`.
...
"`¬` means \"it is not the case that\".  To enter the symbol `¬`,
type `\\not`."
...
some statement `P`, then `exact h2 h1` will close the goal.
"
